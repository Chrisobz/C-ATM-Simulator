...
ATM SIMULATOR
			...
			Checkings Account Balance = 200
			Savings Account Balance = 400
*/
...
bool proceed(string response) {
	if (response == "y" || response == "Y") {
		...
		string exit;
		cout << "\n\n\n\t Thank you for banking with Chris's bank....See you next time.";
		cin >> exit;
		return false;
		exit;
		cin.clear();
...
public:
	...
	int getTransfer() {
		...
		if (AmountTransfer <= AccountDetails[this->type]) {
			...
			// set the new amount to transfered account
			...
			cout << "Insuffcient funds." << endl; // display insufficient funds if the right amount of money is not available
			getBalance();
		}
...
void bank(int option) {
	cout << "The nearest bank is 1.5 miles away at 23 Crescent close, N45 C4E3" << endl;
...
int main() {
	int pin;
	cout << "Welcome to Chris's Bank ATM. Your pin is 1234\n" << endl;
	cout << "DO NOT ADD SPACES OR EXTRA CHARACTERS" << endl;
	...
	do {
		...
		if (validatePin(pin)) {
			...
			cin.clear();
			cin.ignore(1000, '\n');
			cin.ignore(pin || 1000); // ignore character with pin e.g 1234e
		}
